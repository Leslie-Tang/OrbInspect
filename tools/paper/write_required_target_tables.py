...
from publication_labels import profile_label, objective_label
...
def main() -> None:
    ...
    out = ROOT / 'OrbInspectLatex' / 'tables'
    ...
    for split, display in (('test', 'Test'), ('ood', 'Shifted')):
        ...
        for p in sorted(set(profile(r) for r in development)):
            ...
            profile_lines.append(f'{profile_label(p)} & {required} & {display} & '
        ...
        for method in sorted(set(r['method'] for r in subset) - set(METHODS)):
            ...
            objective_lines.append(f'{display} & {objective_label(method)} & {completed}/{len(diagnostic)}')
    ...
    for p in requirements['profiles']:
        ...
        profile_manifest.append(f'{profile_label(p["profile_id"])} & {p["required_target_count"]} & {region_counts} & {ids}')
