...
def audit_target_events(events: list[dict], views: list[dict], mission: dict) -> dict:
    ...
    mode = mission.get('goal_mode', 'required')
    if mode not in ('required', 'hybrid'):
        raise ValueError('audit requires a required-target or hybrid mission')
    background_goal = float(mission.get('goal_coverage', 0.0))
    if mode == 'hybrid' and not 0.0 < background_goal <= 1.0:
        raise ValueError('hybrid mission requires a positive background goal')
    ...
    for sequence, (event, view) in enumerate(zip(events, views, strict=True), 1):
        ...
        goal_reached = not required - accepted and (
            mode == 'required' or coverage + 1e-12 >= background_goal)
        consistent = (
            ...
            and bool(event['mission_goal_reached']) == goal_reached
            and event['mission_sha256'] == mission['mission_sha256']
            and event['goal_mode'] == mode
    ...
    return {
        ...
        'goal_mode': mode, 'goal_coverage': background_goal,
        'accepted_background_coverage': sum(weights[k] for k in sorted(accepted)) / sum(weights.values()),
...
def audit_run(run: Path) -> dict:
    ...
    if mission['goal_mode'] not in ('required', 'hybrid'):
        raise ValueError('this audit requires an explicit required-target or hybrid mission')
    ...
    gates = {
        ...
        'background_goal_accepted': (mission['goal_mode'] == 'required' or
                                    result['accepted_background_coverage'] + 1e-12 >= mission['goal_coverage']),
