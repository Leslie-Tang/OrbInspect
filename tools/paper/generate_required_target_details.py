...
from paper_paths import figure_directory
from publication_labels import REPRESENTATIVE_CASE_LEAD
import shutil
...
def main() -> None:
    ...
    def save(fig, name):
        for suffix in ('pdf', 'png', 'svg'):
            ...
            fig.savefig(path)
            shutil.copy2(path, figure_directory(name) / path.name)
    ...
    (ROOT/'OrbInspectLatex/tables/required_target_case.tex').write_text(text)
