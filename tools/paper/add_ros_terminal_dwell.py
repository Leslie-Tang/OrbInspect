...
def add_dwell(source: Path, destination: Path, seconds: float, mean_motion: float,
              *, selection_reason: str = 'Post-hoc execution adaptation after frozen route tracking failure; offline study unchanged') -> Path:
    ...
    manifest['execution_schedule'] = dict(
        ...
        graph_cost_basis='unchanged source transfers only; excludes settling time and feedback',
    ...
    (destination/'summary.md').write_text(
        ...
        'unchanged source transfer. Observations evaluated after dwell. This is not execution evidence.\n')
    return destination
