...
def main() -> None:
    ...
    figure_note = ('Figure 1 is a concise four-stage visual overview with an archived graph, '
                   'a schematic ADP tree, an archived planned-route projection, and two independently '
                   'embedded photographs. Detailed definitions remain in the manuscript; image credits and original '
    ...
    for note in ('ieee_required_target_revision_plan_20260905.md','required_target_reproducibility_notes_20260905.md',
                 ...
                 'figure1_technical_revision_20260906.md',
                 'figure1_visual_revision_20260906.md'):
...
'''
    ...
    if editable_figures:
        content += '''\n## Editable framework figure and prose cleanup\n\nFigure 1 is `OrbInspectLatex/figures/required_target/orbinspect_framework.drawio`. Open it in draw.io to edit native shapes, short labels, graph edges, the ADP tree, trajectory curves, and connectors. Matching SVG and mixed-media PDF exports are included. The paper uses the PDF across two columns; Figures 2–7 and the numerical results are unchanged. See `docs/figure1_visual_revision_20260906.md` for the concise visual revision, `docs/figure1_technical_revision_20260906.md` for the preceding logic audit, and `docs/manuscript_cleanup_20260906.md` for the earlier figure-reference audit and prose consolidation.\n\nThe graph and planned-route projection are extracted from frozen archives; the ADP tree is schematic. `figures/required_target/framework_assets/VISUAL_EXAMPLES.json` records exact source hashes, graph edges, route samples, and required-target positions. The earlier `TECHNICAL_EXAMPLES.json` belongs to the preceding text-heavy design. The ISS and camera photographs remain independently replaceable, unchanged image objects with their original credits.\n\nTo rebuild the current diagram, install PyMuPDF and run:\n\n```bash\npython OrbInspectLatex/scripts/generate_visual_framework.py\n```\n\nThe current generator imports shared native geometry from `generate_editable_framework.py`; running that older helper directly would regenerate the preceding verbose layout. Manual edits made subsequently in draw.io should instead be exported from draw.io and must not be overwritten by regeneration.\n'''
