...
The planner operates offline. It receives a known station mesh, an initial relative state, camera parameters, a keep-out model, and actuation limits, and returns a time-indexed translational reference, a camera-boresight stream, selected SOOA records, and summary metrics. Section~\ref{sec:ros-verification-protocol} describes ROS~2 closed-loop execution of those records, and Section~\ref{sec:ros-verification-results} reports the corrected validation task separately from the archived graph metrics. Table~\ref{tab:assumptions} states the modeling assumptions and their implications for interpreting both evidence layers.
...
Development considered a frozen fitted critic, a hand-scaled critic heuristic, a fixed-incumbent-suffix rollout, and repeated adaptive rollout. On the full 12-scenario validation set, adaptive depths one, two, and three all reached 12/12 goals; their mean graph costs were 105.045, 103.879, and 102.444, compared with 109.343 for local search. The predeclared lexicographic rule prioritized success, paired penalized cost, then latency. Depth three won 10 of 12 cost pairs and had the lowest validation mean, so it was selected before the corrected held-out evaluation. Figure~\ref{fig:adp-development} reports this validation-only selection.

\begin{figure*}[t]
  \centering
  \subfloat[Validation graph cost.\label{fig:adp-development-cost}]{%
    \includegraphics[width=0.315\textwidth]{figures/adp_future/adp_development_selection_a.pdf}}
  \hfill
  \subfloat[Paired cost difference.\label{fig:adp-development-difference}]{%
    \includegraphics[width=0.315\textwidth]{figures/adp_future/adp_development_selection_b.pdf}}
  \hfill
  \subfloat[Median online time.\label{fig:adp-development-time}]{%
    \includegraphics[width=0.315\textwidth]{figures/adp_future/adp_development_selection_c.pdf}}
...
\begin{figure*}[!t]
  ...
  \subfloat[Test physical $\Delta v$.\label{fig:paired-performance-test-dv}]{%
    \includegraphics[width=0.40\textwidth]{figures/adp_future/adp_heldout_performance_a.pdf}}
  \hfill
  \subfloat[Test graph-cost differences.\label{fig:paired-performance-test-cost}]{%
    \includegraphics[width=0.45\textwidth]{figures/adp_future/adp_heldout_performance_b.pdf}}
  \\[-1ex]
  \subfloat[Shifted physical $\Delta v$.\label{fig:paired-performance-shifted-dv}]{%
    \includegraphics[width=0.45\textwidth]{figures/adp_future/adp_heldout_performance_c.pdf}}
  \hfill
  \subfloat[Shifted graph-cost differences.\label{fig:paired-performance-shifted-cost}]{%
    \includegraphics[width=0.45\textwidth]{figures/adp_future/adp_heldout_performance_d.pdf}}
...
\begin{figure*}[!t]
  ...
  \subfloat[Test $\Delta v$ distributions.\label{fig:paired-safety-dv}]{%
    \includegraphics[width=0.48\textwidth]{figures/adp_future/adp_ablation_safety_a.pdf}}
  \hfill
  \subfloat[Mission success by method.\label{fig:paired-safety-success}]{%
    \includegraphics[width=0.48\textwidth]{figures/adp_future/adp_ablation_safety_b.pdf}}
  \\[0.5ex]
  \subfloat[Minimum clearance above margin.\label{fig:paired-safety-clearance}]{%
    \includegraphics[width=0.48\textwidth]{figures/adp_future/adp_ablation_safety_c.pdf}}
  \hfill
  \subfloat[Peak commanded input.\label{fig:paired-safety-input}]{%
    \includegraphics[width=0.48\textwidth]{figures/adp_future/adp_ablation_safety_d.pdf}}
...
\begin{figure*}[!t]
  ...
  \subfloat[Generated LVLH routes.\label{fig:representative-route}]{%
    \includegraphics[width=0.315\textwidth]{figures/adp_future/adp_representative_trajectory_a.pdf}}
  \hfill
  \subfloat[Coverage accumulation.\label{fig:representative-coverage}]{%
    \includegraphics[width=0.315\textwidth]{figures/adp_future/adp_representative_trajectory_b.pdf}}
  \hfill
  \subfloat[Cumulative physical $\Delta v$.\label{fig:representative-dv}]{%
    \includegraphics[width=0.315\textwidth]{figures/adp_future/adp_representative_trajectory_c.pdf}}
...
The corrected route exporter reconstructs all 124 selected ADP/local routes and aborts on any mismatch with archived success, coverage, graph cost, $\Delta v$, clearance, or peak input. The result supports a bounded statement: within the frozen SOOA library and incumbent-feasible scenario generator, depth-three rollout ADP improves the strongest local baseline on average while retaining sampled graph viability. It does not establish superiority for every pair, for scenarios without a feasible base completion, for different meshes or transfer durations, or under navigation, actuator, and model uncertainty.
