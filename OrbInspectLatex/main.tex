...
\begin{abstract}
Autonomous orbital inspection couples a history-dependent surface-coverage objective with dynamically reachable and auditable motion. We formulate the problem on a directed candidate graph whose observation nodes store camera-valid target masks and whose safe observable orbital arc (SOOA) edges store finite-duration Hill--Clohessy--Wiltshire transfers and common safety audits. The main contribution is a viability-preserving rollout approximate dynamic programming (ADP) policy. At every decision, a finite-depth Bellman lookahead evaluates all shield-admissible actions and appends a deterministic safe-greedy completion as an approximate terminal value; infeasible completions receive infinite value. This construction supplies policy improvement without depending on a learned critic and preserves a goal-reaching continuation in the deterministic graph. A frozen development study compared fitted-critic, heuristic, fixed-rollout, and adaptive-rollout structures and selected depth three before held-out evaluation. On 30 incumbent-feasible test scenarios, the proposed policy and the strongest local-search baseline both reached the 80\% inspectable-coverage goal in all cases. Rollout ADP reduced mean $\Delta v$ from 12.946 to \unit{11.239}{m/s} (13.18\%); the paired difference was \unit{-1.707}{m/s} with a 95\% bootstrap interval $[-2.199,-1.265]$, 28 wins, two numerical ties, and no losses ($p=7.45\times10^{-9}$, exact sign test). The direction persisted in 20 shifted scenarios, with an 8.37\% $\Delta v$ reduction. Full-mesh queries use all 247,525 ISS triangles. The evidence is deterministic and simulator-only, is conditioned on incumbent feasibility, and does not establish continuous-time or closed-loop flight safety.
\end{abstract}
...
The finite representation makes sequential optimization and verification explicit. A node identifier alone is not Markov because the value of a view depends on what has already been covered and how much budget remains. We therefore augment the state with covered- and selected-set masks and develop viability-preserving rollout ADP. A shield-admissible safe-greedy policy supplies an approximate cost-to-go; finite-depth Bellman expansion improves the first action, the state is updated, and the procedure repeats until the coverage goal is reached. Exact dynamic programming remains available only for reduced graphs small enough to exhaust.
...
\begin{enumerate}
  ...
  \item a split-controlled algorithm-development and held-out study that rejects an unsuccessful fitted critic, freezes rollout depth on validation, and reports paired effect sizes, confidence intervals, exact sign tests, shifted-scenario results, and exact non-timing reproducibility.
\end{enumerate}
...
Let $g(s,a)$ denote the marginal inspectable-coverage gain of action $a$ and let $\epsilon=0.05$ prevent division by a near-zero stage cost. The deterministic adaptive base policy is
\begin{equation}
...
\end{equation}
with deterministic node-identifier tie breaking. It recomputes gain after every accepted observation. Starting from state $s$, its completion value is
\begin{equation}
  \widehat V_0(s)=
  \begin{cases}
...
\State $s\gets(v_0,\mathbf0,\mathbf0,H)$; $\pi\gets()$
\While{$C^{\mathrm{insp}}(m)<C_{\mathrm{req}}^{\mathrm{insp}}$ and $h>0$}
  \State Enumerate every $a\in\mathcal U_s(s)$
  \For{each admissible $a$}
    \State $Q_d(s,a)\gets\ell_{ja}+\widehat V_{d-1}(f(s,a))$
  \EndFor
  \State Discard every action with $Q_d(s,a)=+\infty$
  \If{no finite action remains} \State \Return infeasible \EndIf
  \State $a^\star\gets\arg\min_a Q_d(s,a)$ with deterministic ties
...
\begin{IEEEproof}
Finiteness in Eq.~\eqref{eq:rollout-value} implies that at least one admitted first action has a finite depth-$(d-1)$ continuation. At depth zero, finiteness is assigned only when the simulated base policy reaches the goal. After executing the minimizing first action, the successor therefore has a finite completion. The graph, masks, costs, and audits are deterministic, so the same completion remains available when the tree is rebuilt. Induction over the decreasing action budget proves goal attainment.
\end{IEEEproof}

\begin{proposition}[Base-policy bound for the rollout value]
\label{prop:base-bound}
If the adaptive base policy reaches the goal from $s$, then $\widehat V_d(s)$ is finite and $\widehat V_d(s)\le\widehat V_0(s)$ for every $d\ge1$.
\end{proposition}

\begin{IEEEproof}
The first base-policy action belongs to $\mathcal U_s(s)$ and its successor has a finite base completion. It is therefore a feasible candidate in Eq.~\eqref{eq:rollout-value}, giving $\widehat V_1(s)\le\widehat V_0(s)$. Repeating the argument and induction on $d$ establishes the result. The proposition bounds the deterministic rollout surrogate; reductions in physical $\Delta v$ are evaluated separately.
\end{IEEEproof}
...
With maximum branching factor $B$, depth $d$, and at most $H$ base-policy actions, a direct decision evaluation is $O(B^dH B)$ in the worst case; memoization is effective when different prefixes reach the same mask state. The reported $d=3$ setting is consequently an offline planner. Runtime is measured, but it is not used as evidence of real-time capability.
...
Development considered a frozen fitted critic, a hand-scaled critic heuristic, a fixed-incumbent-suffix rollout, and repeated adaptive rollout. An exact-action-advantage ridge route failed an initial validation success screen and was rejected. On the full 12-scenario validation set, adaptive depths one, two, and three all reached 12/12 goals; their mean graph costs were 101.424, 97.388, and 93.586, compared with 109.299 for local search. The predeclared lexicographic rule prioritized success, paired penalized cost, then latency. Depth three won all 12 cost pairs and was frozen before either test split was opened.
...
The primary claim gate is passed. On the test split, the ADP-minus-local graph-cost difference is $-14.016$ with 95\% interval $[-18.265,-10.451]$ and $d_z=-1.274$. This is a 13.12\% reduction relative to the local mean. The corresponding physical $\Delta v$ difference is \unit{-1.707}{m/s}, interval $[-2.199,-1.265]$, $d_z=-1.274$, or 13.18\%. Both endpoints have 28 wins, two numerical ties, no losses, and exact sign-test $p=7.45\times10^{-9}$. Mean selected-count difference is zero. Thus the advantage is route quality, not a comparison between a short failed path and a complete mission.
...
A flight-oriented development path should first validate closed-loop replay on Ubuntu~24.04 with ROS~2 Jazzy, since the current ROS section is intentionally a placeholder. The next algorithmic study should deliberately include incumbent-infeasible scenarios and compare viability repair, beam rollout, and constrained fitted values under a fixed test protocol. Batched full-mesh edge evaluation and cached passive audits can then support passive-drift experiments. Uncertainty-inflated keep-out volumes, camera-slew and illumination constraints, navigation error, a rotating body frame, and an optional Basilisk backend should follow while retaining HCW as a transparent regression reference.
...
On 30 frozen, incumbent-feasible test scenarios, rollout ADP and local search both reached the 80\% inspectable-coverage goal. ADP reduced mean $\Delta v$ by 13.18\%, from 12.946 to \unit{11.239}{m/s}; the paired mean difference was \unit{-1.707}{m/s}, with a 95\% interval $[-2.199,-1.265]$, 28 wins, two ties, and no losses. An 8.37\% reduction persisted under stronger node dropout and target-priority shift. The fitted-critic route failed the success gate, while the model-based rollout structure was exactly reproducible in all non-timing fields. These results make rollout ADP---not the fitted critic or local reversal search---the main contribution. The conclusion remains limited to a deterministic sampled graph and does not claim passive safety, continuous-time robustness, continuous-space optimality, or ROS execution performance.

\section*{Data and Code Availability}
The experiment configuration, archived graph and scenarios, raw method rows, statistical outputs, figure source tables, transformation hashes, and deterministic rerun report are included in the accompanying OrbInspect project package under the paper-grade result bundles. A public archival identifier should be added before final submission.
