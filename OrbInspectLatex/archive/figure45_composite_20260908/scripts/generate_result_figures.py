#!/usr/bin/env python3
"""Render Figures 4/5 at final column size without rerunning experiments.
...
existing palette, explicit units, embedded PDF fonts and editable SVG text.
"""
...
RED, ROSE, PURPLE, BLUE = '#750014', '#E3CCD0', '#7F3F98', '#587E92'
ORANGE, GREY, BLACK = '#E46240', '#6B7280', '#1E293B'
...
def plot_performance(evidence: dict):
    ...
    for index, (split, color, marker, name) in enumerate((
            ('test', RED, 'o', 'Test'), ('ood', PURPLE, 's', 'Shifted'))):
        ...
        b.axhspan(*cohort['bootstrap_95_ci'], color=ROSE, alpha=.55)
        b.axhline(cohort['mean_graph_cost_difference'], color=RED, linewidth=1)
        b.grid(False, axis='x')
...
def plot_safety(evidence: dict):
    ...
    bars = a.bar(np.arange(3), means, yerr=np.vstack((means-ci[:,0], ci[:,1]-means)),
                 capsize=2, color=(PURPLE, BLUE, RED), edgecolor=BLACK, linewidth=.5,
    ...
    for offset, split, color, hatch in ((-width/2,'test',RED,''), (width/2,'ood',PURPLE,'//')):
    ...
    for ax, field in ((c, 'min_clearance'), (d, 'peak_input')):
        for split, marker, color in (('test', 'o', RED), ('ood', 's', PURPLE)):
        ...
        ax.axhline(0 if field == 'min_clearance' else .06,
                   color=RED if field == 'min_clearance' else ORANGE, linewidth=.8)
    ...
    handles = [(Patch(facecolor=color, hatch=hatch, edgecolor=BLACK, linewidth=.5),
               ...
               for color, hatch, marker in ((RED, '', 'o'), (PURPLE, '//', 's'))]
    fig.legend(handles, ['Test', 'Shifted'], handler_map={tuple: HandlerTuple(ndivide=None)},
               loc='center', bbox_to_anchor=(.5, .515), ncol=2, frameon=False,
...
def main() -> None:
    ...
    for number, directory, stem, builder in (
        ...
        report = export(builder(evidence), destination, stem)
        manifest = {'number': number, 'layout': 'single-column 2x2', 'geometry': report,
                    'generator_sha256': sha(Path(__file__)), 'evidence': evidence}
        (destination/'single_column_manifest.json').write_text(json.dumps(manifest, indent=2)+'\n')
        outputs.append({'figure': number, **report})
