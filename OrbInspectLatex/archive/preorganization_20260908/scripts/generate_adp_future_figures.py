...
def plot_rollout_architecture(
    result_dir: Path, paper_dir: Path, required_targets: bool = False,
) -> list[Path]:
    ...
    boxes = [
        (0.02, 0.25, 0.17, 0.50, 'Markov state',
         '$s_k=(j_k,m_k,$\n$b_k,h_k)$\n'
         + ('fixed required set $K$' if required_targets else 'masks + budget'), BLUE),
        ...
        (0.45, 0.14, 0.28, 0.72, 'Depth-3 rollout ADP',
         ...
         + ('all required targets' if required_targets else 'feasible completion'), RED),
    ...
    ax.text(0.59, 0.93, 'only audited, certified actions can be selected',
