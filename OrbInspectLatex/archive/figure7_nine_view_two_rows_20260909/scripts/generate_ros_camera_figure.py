...
def generate(output: Path, data: Path = DATA, font_dir: Path | None = None) -> dict:
    ...
    for index, (frame, record) in enumerate(zip(frames, records, strict=True)):
        ...
        x = CAMERA_LEFT_MM + col * column_pitch
    ...
    xz_rect = [10.6, 11.0, (limits['x'][1] - limits['x'][0]) * mm_per_m, projection_height]
    yz_rect = [37.0, 11.0, (limits['y'][1] - limits['y'][0]) * mm_per_m, projection_height]
    ...
    report = {
        ...
        'layout': 'Two shared-axis projections on the left; five camera columns in two rows on the right; legend in the spare tenth cell when nine views are present',
        ...
        'correspondence': f'Camera top row 1-5 matches projection (a); camera bottom row 6-{count} matches projection (b); inactive hollow markers retained',
