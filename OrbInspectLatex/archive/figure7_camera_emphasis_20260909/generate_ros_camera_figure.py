...
WIDTH_MM, HEIGHT_MM = 170.0, 72.0
CAMERA_WIDTH_MM = 24.0
CAMERA_GAP_MM = 1.0
CAMERA_ROW_PITCH_MM = 23.3
...
def camera_label(record: dict) -> str:
    ...
    if 'required_target_count' in record:
        return (f"{time} · Req {record['accepted_required_count']}/"
...
def generate(output: Path, data: Path = DATA, font_dir: Path | None = None) -> dict:
    ...
    if count not in (9, 10):
        raise ValueError('The approved compact layout supports nine or ten observations')
    assert [r['sequence'] for r in records] == list(range(1, count + 1))
    if snapshot.get('goal_mode') == 'required':
    ...
    camera_axes = []
    width = CAMERA_WIDTH_MM
    column_pitch = width + CAMERA_GAP_MM
    for start, left in ((0, 0.7), (5, WIDTH_MM - 0.7 - width - column_pitch)):
        for local in range(min(5, count - start)):
            row, col = divmod(local, 2)
            x = left + col * column_pitch if local < 4 else left + column_pitch / 2
            image_height = width * frames[start + local].shape[0] / frames[start + local].shape[1]
            y = HEIGHT_MM - 1.0 - image_height - row * CAMERA_ROW_PITCH_MM
    ...
    xz_rect = [58.8, 26.0, (limits['x'][1] - limits['x'][0]) * mm_per_m, projection_height]
    yz_rect = [94.5, 26.0, (limits['y'][1] - limits['y'][0]) * mm_per_m, projection_height]
    xz = add_axes_mm(figure, *xz_rect)
    yz = add_axes_mm(figure, *yz_rect)
    planned_line, executed_line = projection(
        xz, arrays, 0, range(5), '(a) $x$-$z$ projection', limits)
    projection(yz, arrays, 1, range(5, count), '(b) $y$-$z$ projection', limits)
    ...
    figure.legend(handles, ('Planned', 'Executed', 'Start'), loc='lower center',
                  bbox_to_anchor=(0.515, 8.0 / HEIGHT_MM), ncol=3, frameon=False, fontsize=7.2,
    ...
    if 'minimum_body_clearance_m' in snapshot:
        ...
        margin_label = f'Min. full 3-D body margin: {margin:+.2f} m'
    else:
    ...
    figure.text(0.515, 16.0 / HEIGHT_MM,
                margin_label,
    ...
    figure.savefig(output / f'{STEM}.svg', dpi=600)
    figure.savefig(output / f'{STEM}.pdf', dpi=600)
    ...
    report = {
        ...
        'layout': 'Two camera columns on each side, centered fifth view when present; two central projections',
        'camera_width_mm': CAMERA_WIDTH_MM,
        'camera_column_gap_mm': CAMERA_GAP_MM,
        'camera_row_pitch_mm': CAMERA_ROW_PITCH_MM,
        'projection_spatial_scale_relative_to_previous_layout': PROJECTION_SCALE,
        ...
        'correspondence': f'Matching observation numbers; active 1-5 left and 6-{count} right; inactive hollow markers retained',
