...
def named_chaser_pose(message: dict) -> dict | None:
    """Distinguish empty startup scenes from valid poses and reject duplicates."""
    poses = [pose for pose in message.get('pose', ()) if pose.get('name') == 'chaser']
    if len(poses) > 1:
        raise ValueError('Multiple named Gazebo chaser poses in one scene message')
...
def prepare(run: Path, capture: Path, output: Path) -> dict:
    ...
    scene = []
    empty_scene_records = []
    scene_record_count = 0
    with (capture/'gazebo_scene_poses.jsonl').open() as stream:
        for scene_index, line in enumerate(stream):
            scene_record_count += 1
            record = json.loads(line)
            message = record['message']
            header = message['header']['stamp']
            t = int(header.get('sec', 0)) + int(header.get('nsec', 0))*1e-9
            chaser = named_chaser_pose(message)
            if chaser is None:
                empty_scene_records.append(dict(record_index=scene_index, simulator_stamp_s=t,
                                                reason='No named chaser in recorded scene'))
                continue
            p, q = chaser['position'], chaser['orientation']
            scene.append((t, [p.get(k, 0) for k in 'xyz'],
                          [q.get(k, 0) for k in 'xyzw'], record['wall_receipt_ns']*1e-9))
    scene_diagnostics = dict(total_records=scene_record_count, valid_chaser_records=len(scene),
                             records_without_chaser=empty_scene_records,
                             handling='Raw records retained; no pose synthesized. Each selected frame must still pass the unchanged bracketing and pose gates.')
    (output/'scene_pose_stream_diagnostics.json').write_text(json.dumps(scene_diagnostics,indent=2)+'\n')
    ...
    for i, (event, frame) in enumerate(zip(events, selected, strict=True)):
        ...
        record = dict(sequence=i+1, waypoint_id=event['current_waypoint_id'],
                      ...
                      gazebo_chaser_orientation_xyzw=scene_pose[2],
                      event_odom_orientation_xyzw=odom[3],
    ...
    camera_audit = dict(
        ...
        scene_pose_stream_diagnostics=scene_diagnostics,
    ...
    snapshot = dict(schema='orbinspect-required-target-camera-snapshot/v1', source_run=str(run),
                    goal_mode=audit.get('goal_mode', 'required'),
                    goal_coverage=audit.get('goal_coverage', 0.0),
                    execution_audit_passed=True, camera_audit_passed=True,
