...
def add_dwell(source: Path, destination: Path, seconds: float, mean_motion: float) -> Path:
    ...
    for filename in ('trajectory.csv', 'attitude.csv', 'control.csv', 'viewpoints.csv',
        ...
        for i, row in enumerate(rows):
            row = dict(row)
            action = int(row.get('action') or min(route['action_count'], round(float(row['time'])/90)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mean-motion', type=float, default=0.00113137)
    args = parser.parse_args()
    print(add_dwell(args.source, args.destination, args.seconds, args.mean_motion))
