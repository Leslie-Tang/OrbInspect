...
WAYPOINT_MARKER_AREA_PT2 = 57.0
WAYPOINT_LABEL_FONT_PT = 5.2
...
def projection(axis: mpl.axes.Axes, arrays: dict, horizontal: int,
    ...
    for index, (point, color) in enumerate(zip(points, COLORS[:len(points)], strict=True)):
        ...
        axis.scatter(point[horizontal], point[2], s=WAYPOINT_MARKER_AREA_PT2 if is_active else 26,
        ...
        # Smaller labels leave space inside the circle for two-digit view IDs;
        # hollow inactive markers provide context without repeated numbers.
        if is_active:
            axis.annotate(str(index + 1), xy=(point[horizontal], point[2]),
                          ha='center', va='center', fontsize=WAYPOINT_LABEL_FONT_PT, fontweight='bold',
...
def generate(output: Path, data: Path = DATA, font_dir: Path | None = None) -> dict:
    ...
    report = {
        ...
        'minimum_annotation_font_pt': WAYPOINT_LABEL_FONT_PT,
        'waypoint_marker_area_pt2': WAYPOINT_MARKER_AREA_PT2,
        'waypoint_marker_diameter_pt': float(np.sqrt(WAYPOINT_MARKER_AREA_PT2)),
        'waypoint_label_font_pt': WAYPOINT_LABEL_FONT_PT,
